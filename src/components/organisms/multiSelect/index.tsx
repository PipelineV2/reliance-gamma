  ...
  const [showDropdown, setShowDropdown]= useState<boolean>(false)
    ...
    useEffect(() => {
      if (searchQuery !== '' && typeof options[0] === 'object') {
        setDropdownOptions(options?.filter(({id, name}: optionsProps) => name?.toLocaleLowerCase()?.includes(searchQuery?.toLocaleLowerCase())))
      }
    ...
    return (
        <div className={`${selectClassName}`} style={selectStyle}>
            <div className={`${dropdownClassName}`} style={dropdownStyle} onClick={()=>{
                ...
                <div>{selected.length > 0 ? `${selected.length} plan${selected.length > 1 ? 's':''} selected`: "Select Plan" }</div>
            ...
            <ul className={`${optionsContainerClass}`} style={optionsContainerStyle}>
                { typeof options[0] === 'object' && dropdownOptions.map(({id, name}: optionsProps) => {
                    const isSelected = selected.includes(name);
                    return (
                        <li key={id} className={`${optionsClass}`} style={optionsStyle} onClick={() => onChange(name)}>
